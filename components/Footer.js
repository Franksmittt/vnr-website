...
const FooterLink = ({ href, children }) => (
  <li>
    <Link
      href={href}
      // FIX: CONTRAST - Changed text-slate-400 to text-slate-300
...
const SocialIcon = ({ href, children, label }) => (
  <a
    ...
    // FIX: NAMES & LABELS - Added aria-label for the icon link
    aria-label={label}
    // FIX: CONTRAST - Changed text-slate-400 to text-slate-300
...
const Footer = () => (
  <footer className="bg-slate-800 text-white">
    ...
    <div className="bg-slate-900 py-4">
      {/* FIX: CONTRAST - Changed text-slate-500 to text-slate-400 */}
